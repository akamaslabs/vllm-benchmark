...
| 17 | [17-g7e-speculative-decoding-goodput](studies/17-g7e-speculative-decoding-goodput/README.md) | vLLM (single GPU, speculative decoding) | **First study to actually test speculative decoding, scaffolded 2026-09-21** — duplicates #2's "2-Larger-goodput" (same `g7e.4xlarge` / RTX PRO 6000 Blackwell, same `Qwen/Qwen2.5-7B-Instruct`, same goodput goal, SLA, windowing and 14 tuned parameters) and adds `vLLM.spec_method` (`none`/`ngram`/`ngram_gpu`) and `vLLM.spec_tokens` `[0,16]`, for 16 parameters and 7 `parameterConstraints`. Motivated by #16's finding that its best configurations sat against the GPU power cap with the memory bus ~48% active and tensor cores ~11% — idle compute is exactly what a drafter spends. Addresses all 5 of #1b's speculative-decoding crashes: `mtp` and `suffix` are excluded from the domain (both failed 100% of the time, and a categorical always-fails value cannot be fenced by a constraint), `ngram_gpu`+`optimization_level 0` and `ngram`+`async_scheduling` are fenced by constraints, and the off-sentinel is deleted from the rendered command line by `apply_config.sh` because vLLM only reads speculative decoding as off when the flags are ABSENT. Three further deliberate changes: vLLM image 0.22.0 -> 0.29.0 (the pack's reference version, and the only one emitting the V1 spec-decode counters), the AIPerf ramp re-floored 150 -> 8 (a sweep starting at 150 observes only the saturated regime where speculation is expected to lose, answering the question by construction), and 7 preset steps running the whole method x token-depth grid head-to-head before the optimizer, since the throughput goal is expected to reject speculation regardless of how well it works. **Known gap:** acceptance rate is not measurable — pack 1.9.1 declares no speculative-decoding metric, so the four real V1 counters sit commented out in the telemetry instance pending a pack change. | TODO |
