...
| 11 | [11-gpt-oss-20b-tps-peak](studies/11-gpt-oss-20b-tps-peak/README.md) | vLLM (MoE model, 4-GPU topology + expert parallelism) | **Short-ramp twin of #10, scaffolded 2026-09-15 to make the search affordable**: identical model (`openai/gpt-oss-20b`), node, goal, SLA, 14 parameters, domains and 5 `parameterConstraints` — only the AIPerf load profile changes, from 12 levels x 300s (~67 min of load per trial, ~10 days for 200 experiments) to 4 log-spaced levels 128/256/512/1024 x 90s (~6 min per trial, ~11 h for 60). The ramp still ends at 1024 because the knee sits **at** `max_num_seqs`: #10's baseline measured it directly on this node (concurrency 253 -> `Waiting=0`, engine-side TTFT p95 < 500 ms; 302 -> 46 queued, TTFT p95 5-7.5 s), so a ceiling below `max_num_seqs`'s [16, 1024] domain would leave configurations tuned toward ~770 (study 1b's winning region) unsaturated and their TPS peak invisible. `stability` windowing kept unchanged on purpose — it picks the best compliant sub-window, so weak configs come out infeasible and strong ones feasible — with only `resolution: 15s` added so the 6-sample window equals one 90s level. | TODO |
| 12 | [12-gpt-oss-20b-steady-throughput](studies/12-gpt-oss-20b-steady-throughput/README.md) | vLLM (MoE model, 4-GPU topology + expert parallelism) | **No ramp at all, scaffolded 2026-09-15**: identical model, node, goal, SLA, 14 parameters and `parameterConstraints` as #10/#11, with AIPerf offering one constant level of 512 concurrent requests for 600s. Removes the three ramp artefacts measured on this node: the ~30s zero-traffic gap AIPerf leaves at every level change, the 90s window that cannot align inside a 90s level (the likeliest cause of #11's baseline reading 2937 vs #10's 3287 for the same configuration), and the mixing of regimes that forces a windowing rule to choose. With one regime, windowing is `trim: [2m, 30s]` on RunTest — bounds measured from the task's own phases — and nothing is selected. 512 is the measured discrimination threshold between configurations that serve the load queue-free and those that breach the TTFT SLA. 45 experiments at ~15.5 min ~= 11.5 h. Absolute scores across #10/#11/#12 are not comparable; only within-study ranking is. | TODO |
| 13 | [13-gpt-oss-20b-tp-goodput-per-gpu](studies/13-gpt-oss-20b-tp-goodput-per-gpu/README.md) | vLLM (MoE model, tensor parallelism over 2-4 GPUs + expert parallelism) | **Multi-GPU-only variant of #10/#12, scaffolded 2026-09-15**: identical model, node, goal, SLA and 14 parameters, but `tensor_parallel_size` is tuned in **[2, 4]** (`TP != 3` -> {2, 4}) and `data_parallel_size` in [1, 2], so the optimizer only compares TP2/DP1, TP2/DP2 and TP4/DP1 — the question is which way of using >= 2 L4s gives the best goodput per active GPU, the single-GPU replica being excluded on purpose. Load and windowing revert to #10 (12 levels x 300 s, `stability` on the prefill peak, ~75 min per experiment, 100/20): with TP >= 2 the aggregate capacity spans 2-4x one L4, so no single constant level discriminates every topology the way #12's 512 did for a single GPU. Carries a new **sampler-warmup OOM guard** `gpu_memory_utilization * 22.03 + max_num_seqs * 0.00375 <= 21.63` — #12's experiment 12 (TP1/DP4/EP, 0.90, `max_num_seqs` 1024) hit the section C failure mode from study 0 on gpt-oss-20b: 786 MiB fp32 `[1024 x 201088]` logits copies allocated by `warmup_kernels` after weights + KV cache, on all 4 GPUs; the guard admits `max_num_seqs` up to 774 at 0.85 and 480 at 0.90 — plus a crash-loop fail-fast in `apply_config.sh` so a deterministic startup crash costs ~3 min instead of the 20-minute progress deadline. **Ran 2026-09-15/16 and was stopped after 16 of 100 experiments** (last state read live on 2026-09-16 was RUNNING with 16 finished; the stop is per study 14's manifest, not re-verified) (best exp. 16: 2345.31 tokens/s per GPU, +18.49% over the 1979.41 baseline — TP2/DP1, EP on, `kv_cache_dtype` auto, `gpu_memory_utilization` 0.80, `max_num_seqs` 1024). Its Sobol initial design aliased `kv_cache_dtype == fp8` with `enable_expert_parallel == false` perfectly across all 15 optimizer experiments, making neither effect identifiable — #14 supersedes it and imports all 16 experiments. | STOPPED |
| 14 | [14-gpt-oss-20b-parallelism-and-kv-goodput-per-gpu](studies/14-gpt-oss-20b-parallelism-and-kv-goodput-per-gpu/README.md) | vLLM (MoE model, full TP/DP topology + expert parallelism + KV dtype) | **Successor of #13, created and started 2026-09-16** (created outside this repo; not re-verified on the instance — the toolbox CLI session lost its Administrator role).** Same model, node, goal, SLA, windowing, workflow and 14 parameters — it literally **reuses #13's system, telemetry instance and workflow**, which is what makes #13's imported experiments comparable with the new ones. Three changes. (1) `tensor_parallel_size` reopened to **[1, 4]** and `data_parallel_size` to **[1, 4]**, so the TP1 cells #13 excluded by domain are reachable again. (2) #13's baseline and experiments 2-16 imported (`baseline from` + `bootstrap`), ~19 h of node time reused. (3) **Sobol's initial design replaced by 10 explicit `preset` steps** with `numberOfInitExperiments: 0` — Sobol starts at `initial_offset = 0` and is deterministic, so #13's alias is a property of the sequence's head, not bad luck. S3 (`fp8` + EP on), S4 (`auto` + EP off) and S10 (Sobol index 1026) are the cells the alias made unreachable; S6/S7 are TP1/DP1 and TP1/DP4; S8/S9 isolate expert parallelism at TP2/DP2. 8 KPIs, of which `total_token_throughput` and `request_queue_time_p95` have no query in the reused telemetry instance and stay empty. ~12.5 h of presets, then optimize 100/20. | RUNNING |
| 15 | [15-qwen3-30b-a3b-parallelism-goodput-per-gpu](studies/15-qwen3-30b-a3b-parallelism-goodput-per-gpu/README.md) | vLLM (MoE model too large for one GPU, TP/DP topology + expert parallelism) | **Phase 1 of the "topology first, interconnect second" plan agreed on 2026-09-17, scaffolded the same day** — the first study here whose model does not fit a single GPU. `Qwen/Qwen3-30B-A3B-Instruct-2507-FP8` (128 experts / top-8, 48 layers, FP8 e4m3 with `weight_block_size [128,128]`, **29.03 GiB** of weights measured from the HF API) on the same 4x L4 `g6.12xlarge` node, same goodput-per-GPU goal, SLA, `stability` windowing and #14's 14 parameters **plus `disable_custom_all_reduce` and `pipeline_parallel_size`** (16 in total). Both additions are about the interconnect: `disable_custom_all_reduce` is the only such knob pack 1.9.1 already models (pinned to `false` in every preset, left to the optimizer afterwards), and pipeline parallelism ships one activation transfer per stage against tensor parallelism's two all-reduces *per layer*, which on a PCIe-only node is the cheapest collective pattern available — leaving it out would have meant a topology study without the layout this hardware most plausibly favours. It brings a seventh constraint (`PP == 1 || async_scheduling == "false"` — vLLM does not support the combination, upstream issue #32701) and takes the reachable topologies from 6 to 11, of which the 15 presets cover 10, including both head-to-head TP-vs-PP pairs at equal `active_gpus` — but its own system, telemetry instance and workflow, **no `baseline from` and no `bootstrap`** (a different model is not on the same scale). Why this model and this variant: studies 10-14 all served a model that fits one L4, so a parallelism result could never be separated from the overhead of splitting a model that needs no splitting; and study 14 established that vLLM **always** shards MoE experts over the whole TP x DP world (no way to replicate them), so `data_parallel_size` on an MoE is not "independent replicas". At 29.03 GiB single-GPU is impossible while 2-GPU (14.5 GiB/GPU) and 4-GPU (7.3 GiB/GPU) layouts both load, so `active_gpus` varies and the per-GPU objective compares half the hardware against all of it; the bf16 sibling (56.87 GiB) would force TP x DP = 4 everywhere and collapse the objective into raw goodput. 6 `parameterConstraints`: the MoE-aware memory fit (written on the *product* TP x DP) excludes single-GPU layouts by itself, the sampler-warmup guard is re-derived for the 151936 vocab (0.00283 GiB/seq), and a new one requires expert parallelism once the model is split over 4 ranks (`moe_intermediate_size` 768 / 4 = 192 is not a multiple of the 128-wide quantization block — an inference from the checkpoint metadata, which `smoke_test.sh`'s `tp4-noep` exists to confirm or refute). Initial design: 15 explicit presets with `numberOfInitExperiments: 0`, covering 10 of the 11 reachable topologies and all 4 cells of `kv_cache_dtype` x `enable_expert_parallel`. AIPerf ramp re-floored from 150 to 24 (KV is 96 KiB/token here, 2x gpt-oss-20b). ~6.5 days of node time. **Ran 2026-09-18 and was stopped the same day after 6 experiments**: exp 1 baseline 607.19 tokens/s per GPU, exp 2 (S1, TP4/DP1) 827.90 (+36.35%), **exp 4 (S3, TP1/DP4) 1415.55 (+133.13%) — the best by a wide margin**, exp 5 (S4, TP2/DP1, two GPUs) 645.45 (+6.30%), exp 3 (S2, TP2/DP2) and exp 6 (S5, TP1/DP2) ERROR, exp 7 ABORTED on stop. Both failures are `data_parallel_size=2`, while DP1 and DP4 start fine at the same settings. The failure is a CUDA OOM in the sampler warmup that its own memory constraint admitted — the constraint assumed the whole 22.03 GiB framebuffer is vLLM's to spend, while ~2.3 GiB per GPU are CUDA context and NCCL buffers outside `gpu_memory_utilization`'s budget (section C). Superseded by #16. | STOPPED |

| 16 | [16-qwen3-30b-a3b-parallelism-goodput-per-gpu-rerun](studies/16-qwen3-30b-a3b-parallelism-goodput-per-gpu-rerun/README.md) | vLLM (same as #15) | **Successor of #15, created 2026-09-18** — same model, node, goal, SLA, windowing, KPIs, 16 parameters, and it **reuses #15's system, telemetry instance and workflow**, which is what makes #15's experiments importable. Two changes. (1) The sampler-warmup constraint is recalibrated on two measured points instead of an assumed copy count: `gpu_memory_utilization * 22.03 + max_num_seqs * 0.0006 <= 19.7`, where the ceiling is the framebuffer **minus** the ~2.3 GiB of CUDA context and NCCL buffers that sit outside vLLM's budget, and the coefficient is the single fp32 `[max_num_seqs, 151936]` tensor that actually failed rather than five copies. (2) The presets drop to `max_num_seqs` 256 (softmax 148 MiB against 446) at unchanged `gpu_memory_utilization` 0.88 — the lever is the batch and not the memory fraction, because the failing allocation scales only with the former while lowering the latter would starve the 2-GPU layouts of KV cache, and those are the layouts the study exists to measure. #15's baseline is imported via `baseline from` and its valid experiments via `bootstrap`; 13 presets remain to run. **Open before starting**: #15's experiment 6 failed and its log was never read (AWS endpoint unreachable), so it is not yet known whether the 2-GPU cells fail on the sampler or on KV-cache capacity — in the second case they need `kv_cache_dtype` pinned to fp8 rather than a smaller batch. | TODO |
| 17 | [17-g7e-speculative-decoding-goodput](studies/17-g7e-speculative-decoding-goodput/README.md) | vLLM (single GPU, speculative decoding) | **First study to actually test speculative decoding, scaffolded 2026-09-21 and reconfigured the same day.** Started as a duplicate of #2's "2-Larger-goodput" (same single `g7e.4xlarge` / RTX PRO 6000 Blackwell 96GB, same goodput goal, SLA, windowing and 14 tuned parameters) and adds `vLLM.spec_method` and `vLLM.spec_tokens` `[0,16]`, for 16 parameters and 7 `parameterConstraints`. Motivated by #16's finding that its best configurations sat against the GPU power cap with the memory bus ~48% active and tensor cores ~11% — idle compute is exactly what a drafter spends. **Compares three drafter families head-to-head** in 10 presets: `none`, `ngram` and `ngram_gpu` at 3/5/8 tokens (prompt-lookup, no second model, pay off only on repetitive text) and `draft_model` at 3/5/8 with a `Qwen/Qwen3-0.6B` drafter (classic speculation, acceptance independent of repetition). Without both families a poor result would be unattributable. **Model is dense `Qwen/Qwen3-32B-FP8`, chosen after retracting an MoE swap.** Weight bytes read per decode step: 7B bf16 ~15.2 GB, Qwen3-30B-A3B-FP8 ~3.35 GB (8 of 128 experts active), Qwen3-32B-FP8 ~32.8 GB. The MoE made the verify step ~4.5x CHEAPER, leaving less for speculation to amortise, and verifying K+1 drafted tokens reads the union of the experts they route to — a cost a dense target does not pay. The checkpoint is hybrid-thinking, so the deployment passes `--default-chat-template-kwargs` to stop reasoning tokens being generated. Image 0.22.0 → 0.29.0, ramp re-floored and raised to 8→2048 in 9 levels, timeouts raised to #16's proven values for this checkpoint. Addresses all 5 of #1b's speculative-decoding crashes: `mtp`/`suffix` excluded from the domain, `ngram_gpu`+`optimization_level 0` and `ngram`+`async_scheduling` fenced by constraints, and the off-sentinel deleted from the rendered command line because vLLM reads speculation as off only when the flags are ABSENT. **Was blocked on vLLM pack 1.10.0** (adds `draft_model` plus the five acceptance metrics; committed on a branch, not pushed, and the install needs an Administrator role). **HARDWARE AND MODEL CHANGED 2026-09-22:** the RTX PRO 6000 became unobtainable (56+ `InsufficientInstanceCapacity` failures in one day, two further node groups rolled back or failed, and a capacity probe found every GPU class above 24 GB empty in all three `us-east-2` zones), so the study moved to **one NVIDIA L4** (`g6.4xlarge`, 22.49 GiB, Ada/SM89, ~300 GB/s) and, since `Qwen3-32B-FP8` needs 30.5 GiB of weights, to **`Qwen/Qwen3-8B-FP8`**. 8B was chosen over 14B and 4B on arithmetic from each repo's own config.json: 14B leaves ~1.7 GiB of KV once the drafter is loaded and fails vLLM's one-request fit check, while 4B has identical layers and KV heads to the 8B so the drafter would be ~27% of a target step and a null result unattributable. vLLM 0.29.0 gives the drafter its OWN KV cache, so the `draft_model` cells run at 256 KiB/token against the ngram cells' 144 and were re-cut to 2/3/4 tokens. Domains re-derived for 22.49 GiB, the sampler-warmup guard re-added (9 constraints now), preset grid moved to `FLASH_ATTN` since the SM120 maturity worry that pinned `TRITON_ATTN` does not apply to Ada, ramp re-floored to 1→128, and `kv_cache_usage_avg` replaced GPU memory bandwidth in the KPIs because this card IS KV-bound. **Pack blocker cleared:** vLLM 1.10.1 is installed. The name still says G7e and is a deliberate misnomer. | TODO |
...
- **`studies/15-qwen3-30b-a3b-parallelism-goodput-per-gpu`** (Qwen3-30B-A3B-Instruct-2507-FP8,
  ...
  entirely outside vLLM's budget.** Measured on two experiments of the same study, same
  0.88 / 768: experiment 4 (TP1/DP4) started and `nvidia-smi` read **22,261 MiB of 23,034
  in use (96.6%)** against a nominal budget of 19.4 GiB; experiment 3 (TP2/DP2) died with
  `CUDA out of memory, tried to allocate 446.00 MiB, 361 MiB free` inside `warmup_kernels`
  → `flashinfer_sample` → `torch.softmax`. 446 MiB is exactly `max_num_seqs x vocab_size`
  in fp32 (768 x 151936 x 4 B).
  Two rules follow. **(1)** A sampler-warmup guard of the shape
  `gpu_memory_utilization x <GiB> + max_num_seqs x <coef> <= <ceiling>` must set the
  ceiling to the framebuffer **minus that overhead** (here 22.03 - 2.3 = 19.7), not to the
  framebuffer itself — study 13/14's version used the full figure and admitted the
  configuration that then OOMed, with 0.07 GiB of nominal margin. **(2)** When such a
  guard fires, the lever is `max_num_seqs`, not `gpu_memory_utilization`: the failing
  allocation scales linearly with the former and not at all with the latter, while
  lowering the latter shrinks the KV cache — on a model whose weights already take 14.5 of
  ~18 usable GiB per card at 2 GPUs, that would starve exactly the layouts under test.
  Also worth carrying: the crash-loop fail-fast added in study 13 turned this failure into
  **7 minutes** instead of the 30-minute progress deadline.

- **`studies/13-gpt-oss-20b-tp-goodput-per-gpu`** (gpt-oss-20b, 4x L4, vLLM 0.29.0,
  2026-09-16): **Akamas' default Sobol initial design can alias two categorical parameters
  perfectly, and the study will not tell you.** In study 13's 15 optimizer experiments
  `kv_cache_dtype == "fp8"` held if and only if `enable_expert_parallel == "false"` — four
  points on one side, eleven on the other, zero points in the two off-diagonal cells. The
  goal, the constraints and the per-experiment scores all look healthy; the confound is
  only visible by tabulating the configurations out of the study export (the contingency
  table is in study 14's README; the export also confirms the optimizer type on those
  experiments is `SOBOL`). The study-14 manifest attributes the degeneracy to Sobol
  starting at `initial_offset = 0` and being deterministic — i.e. a property of the head
  of the sequence rather than bad luck, which would make **every** study keeping the
  default `numberOfInitExperiments: 10` draw the same first points. That generalisation
  is the manifest author's reasoning, not something measured here: what is measured is
  the 2x2 with two empty cells. Two practical rules follow either way. (1) After the init phase of any study with several categorical
  parameters, cross-tabulate them pairwise before trusting an attribution — a perfect
  2x2 with two empty cells means neither effect is identifiable. (2) The fix that
  `studies/14-gpt-oss-20b-parallelism-and-kv-goodput-per-gpu` applies: replace the init
  phase with explicit `preset` steps chosen to fill the empty cells and set
  `numberOfInitExperiments: 0` — any value > 0 makes the campaign service re-run its own
  Sobol bootstrap on top of the presets, landing back on the same points.
...
- **`studies/0-explorative`**: vLLM's own memory accounting (the `gpu_memory_utilization`
  ...
  targets. **Re-observed 2026-09-15 on gpt-oss-20b / L4** (`12-gpt-oss-20b-steady-throughput`,
  experiment 12: TP1/DP4/EP, `gpu_memory_utilization` 0.89995, `max_num_seqs` 1024 → CUDA OOM on
  all 4 GPUs in `warmup_kernels` → FlashInfer top-k/top-p sampling, trying to allocate 786 MiB =
  one fp32 `[1024 x 201088]` logits copy, after weights + 10.4 GiB of KV cache were already
  placed; vLLM's profiler had reported 0.44 GiB of peak activation). The headroom after the KV
  cache is `(1 - gpu_memory_utilization) x 22.03 GiB` regardless of TP/DP/EP, so TP does not
  help. `13-gpt-oss-20b-tp-goodput-per-gpu` fences it with a `parameterConstraint`,
  `gpu_memory_utilization * 22.03 + max_num_seqs * 0.00375 <= 21.63` (5 fp32 copies of 0.767
  MiB per sequence against the memory the budget leaves free) — reusable shape for any model:
  swap 0.00375 for `5 x vocab_size x 4 B` in GiB.
...
- [ ] **PACK REQUEST — NCCL interconnect tunables as vLLM-component parameters** (raised
      2026-09-17, phase 2 of the topology-then-interconnect plan; phase 1 is study #15).
      On a PCIe-only 4x L4 node (no NVLink) the collectives are the suspected ceiling for
      every multi-GPU layout, and NCCL exposes real knobs for them — all set through
      environment variables on the vLLM process, so nothing in vLLM's own CLI covers them:
      `NCCL_ALGO` (Ring vs Tree: Tree cuts latency on small collectives, Ring maximises
      bandwidth on large ones), `NCCL_PROTO` (Simple/LL/LL128 — LL128 is not expected to
      help over PCIe), `NCCL_P2P_LEVEL` (NVL/PXB/PHB/SYS/LOC/PIX — how far peer-to-peer
      may reach), `NCCL_P2P_DISABLE` (forces traffic through the host; note it makes
      `NCCL_P2P_LEVEL` moot, so the two should not be searched independently),
      `NCCL_MIN_NCHANNELS`, `NCCL_BUFFSIZE`. vLLM pack 1.9.1 models none of them
      (`akamas describe optimization-pack vLLM`, 2026-09-17), and the Deployment template
      would need an env-var passthrough alongside the new parameters. Pack work belongs
      outside this repo, via the `akamas-optimization-pack` plugin against
      <https://gitlab.com/akamas/optimization-packs/vllm>. Deliberately NOT bundled into
      study #15: 6 more categorical/integer parameters on top of its 14 would make the
      topology question unanswerable in one campaign.
      Also worth pack-modelling for the same phase, already discussed in the thread:
      `disable_custom_all_reduce` (**already in the pack**, 1.9.1) — but check first
      whether it is a no-op on this node, since vLLM disables custom all-reduce by itself
      without GPU P2P; the "Custom allreduce is disabled" line in study 13/14's Apply-config
      logs, or `nvidia-smi topo -m`, settles it.
