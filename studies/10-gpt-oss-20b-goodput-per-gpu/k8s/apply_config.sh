#!/bin/bash
# Apply-config step for 10-gpt-oss-20b-goodput-per-gpu (runs on the toolbox host via the
# Akamas Executor, after FileConfigurator rendered 01-deployment_template.yaml).
set -e
DEPLOY_FILE=/work/vllm-benchmark/studies/10-gpt-oss-20b-goodput-per-gpu/k8s/01-deployment.yaml
