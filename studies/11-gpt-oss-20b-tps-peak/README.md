# 11-gpt-oss-20b-tps-peak

**Status:** TODO (scaffolded 2026-09-15 from `10-gpt-oss-20b-goodput-per-gpu`; not started yet)
**Dates:** Scaffolded 2026-09-15

## Objective

**Same goal, same model, same hardware and the same 14 tuned parameters as
`10-gpt-oss-20b-goodput-per-gpu` — only the load profile differs.** Study 10 spends ~67
minutes of load per trial walking a 12-level ramp all the way into saturation; this study
reads the **TPS peak** with a 4-level ramp in ~6 minutes, so an experiment costs ~11
minutes instead of ~75 and a 60-experiment run fits in one night instead of ten days.
Everything else is deliberately held constant so the two studies stay comparable.

The goal is unchanged — which vLLM configuration extracts the most **goodput per GPU
actually used** from one `g6.12xlarge` (4x NVIDIA L4 24 GB), serving
**`openai/gpt-oss-20b`**:
...
## What changes vs. study 10, and why

| | `10-gpt-oss-20b-goodput-per-gpu` | **this study** |
|---|---|---|
| concurrency ramp | 12 levels, 150 -> 1024 | **4 levels, 128/256/512/1024** |
| seconds per level | 300 | **90** |
| load per trial | ~67 min | **~6 min** |
| experiment wall-clock | ~75 min | **~11 min** |
| `windowing.stability.resolution` | unset (datasource native) | **`15s`** (window = 90s = one level) |
| `numberOfExperiments` | 200 | **60** |
| goal, SLA, parameters, domains, `parameterConstraints`, baseline | | **all identical** |

**Why the ramp still ends at 1024.** The ceiling has to match `vLLM.max_num_seqs`'s own
tuned domain (`[16, 1024]`), the same reasoning studies 1/2/7/8/9/10 used. Study 10's
baseline measured the mechanism directly on this node (2026-09-15): the knee sits exactly
**at** `max_num_seqs`. At concurrency 253 the scheduler queue was empty (`Waiting=0`) and
engine-side TTFT p95 stayed under 500 ms; at 302 the default cap of 256 left 46 requests
queued (302 - 256) and TTFT p95 jumped to 5-7.5 s while throughput stayed flat. A
configuration the optimizer tunes toward `max_num_seqs ~= 770` — the region
`1-goodput-realistic-load` actually converged on — has its knee near 770, so a ramp
stopping at 512 would never saturate it and its TPS peak would be invisible. Four levels
at ratio 2x span the whole domain.

**Why the windowing rule is unchanged.** `stability` on `prefill_token_throughput` with
`is: max` selects the best compliant sub-window of each trial's sweep rather than a
whole-trial average (see `1-goodput-realistic-load`'s README). It only lands in a
congested window when the configuration walls out *inside* the ramp — which is the
desired behaviour, since that configuration genuinely cannot serve the offered load:
weak configurations come out infeasible, strong ones feasible. `trim` windowing was
considered and rejected for exactly this reason: averaging over the whole ramp would fold
the saturated tail into every trial's TTFT and make even good configurations infeasible.
Only `resolution` is pinned, and only because each level now lasts 90s.

**Known limitation, inherited and unchanged:** `maxStdDev: 300000000` disables the
stability filter (it is in tokens/s), so the window is simply the 6 samples around the
prefill peak. No gpt-oss/L4 data exists yet to calibrate a real threshold; the workflow's
vLLM-restart guard remains the protection against a trial that crashes after its peak.
...
- **Telemetry:** Prometheus (`kube-prometheus-stack`; the `vllm` and `dcgm-exporter` ServiceMonitors scrape at **5 s**, verified on the cluster 2026-09-15 — this is what makes `windowing.stability.resolution: 15s` meaningful), instance
  `Prometheus_11_GPT_OSS_20B_TPS_Peak`, the same 108 metrics as study 9 (`vllm:*`, DCGM per GPU,
...
5. **Toolbox checkout and key:** `git pull` in `/work/vllm-benchmark`; copy the SSH key to
   `/work/vllm-benchmark/studies/11-gpt-oss-20b-tps-peak/akamas/id_rsa`
...
kubectl apply -f studies/11-gpt-oss-20b-tps-peak/k8s/01-pvc-model-cache.yaml
kubectl apply -f studies/11-gpt-oss-20b-tps-peak/k8s/00-pvc.yaml
kubectl apply -f studies/11-gpt-oss-20b-tps-peak/k8s/06-hf-cache-pvc.yaml
kubectl apply -f studies/11-gpt-oss-20b-tps-peak/k8s/02-service.yaml
kubectl apply -f studies/11-gpt-oss-20b-tps-peak/k8s/04-kv-cache-exporter-configmap.yaml
bash studies/11-gpt-oss-20b-tps-peak/k8s/smoke_test.sh        # must end with "0 failed configuration(s)"

# 1. Akamas resources, typed form, dependency order
akamas create system            studies/11-gpt-oss-20b-tps-peak/akamas/system.yaml
akamas create component         studies/11-gpt-oss-20b-tps-peak/akamas/components/vllm.yaml               "vLLM_Benchmark_11_GPT_OSS_20B_TPS_Peak"
akamas create component         studies/11-gpt-oss-20b-tps-peak/akamas/components/container.yaml          "vLLM_Benchmark_11_GPT_OSS_20B_TPS_Peak"
akamas create component         studies/11-gpt-oss-20b-tps-peak/akamas/components/cluster.yaml            "vLLM_Benchmark_11_GPT_OSS_20B_TPS_Peak"
akamas create component         studies/11-gpt-oss-20b-tps-peak/akamas/components/gpu0.yaml               "vLLM_Benchmark_11_GPT_OSS_20B_TPS_Peak"
akamas create component         studies/11-gpt-oss-20b-tps-peak/akamas/components/gpu1.yaml               "vLLM_Benchmark_11_GPT_OSS_20B_TPS_Peak"
akamas create component         studies/11-gpt-oss-20b-tps-peak/akamas/components/gpu2.yaml               "vLLM_Benchmark_11_GPT_OSS_20B_TPS_Peak"
akamas create component         studies/11-gpt-oss-20b-tps-peak/akamas/components/gpu3.yaml               "vLLM_Benchmark_11_GPT_OSS_20B_TPS_Peak"
akamas create component         studies/11-gpt-oss-20b-tps-peak/akamas/components/container_loadtest.yaml "vLLM_Benchmark_11_GPT_OSS_20B_TPS_Peak"
akamas create component         studies/11-gpt-oss-20b-tps-peak/akamas/components/cluster_loadtest.yaml   "vLLM_Benchmark_11_GPT_OSS_20B_TPS_Peak"
akamas create telemetry-instance studies/11-gpt-oss-20b-tps-peak/akamas/telemetry/prometheus.yaml         "vLLM_Benchmark_11_GPT_OSS_20B_TPS_Peak"
akamas create workflow          studies/11-gpt-oss-20b-tps-peak/akamas/11-GPT-OSS-20B-TPS-Peak-Workflow.yaml
akamas create study             studies/11-gpt-oss-20b-tps-peak/akamas/11-GPT-OSS-20B-TPS-Peak.yaml

#    or, bulk form (every file self-describes kind:/system:; same dependency order applies):
akamas create -f studies/11-gpt-oss-20b-tps-peak/akamas/

# 2. check, then start
akamas describe study "11-GPT-OSS-20B-TPS-Peak"      # expect 14 parameters, 5 parameterConstraints, 2 steps
akamas start study "11-GPT-OSS-20B-TPS-Peak"
akamas list experiment "11-GPT-OSS-20B-TPS-Peak"
```
