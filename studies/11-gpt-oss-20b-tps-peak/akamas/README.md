# akamas/ — Akamas resources of 11-gpt-oss-20b-tps-peak
...
|---|---|
| `system.yaml` | new system `vLLM_Benchmark_11_GPT_OSS_20B_TPS_Peak` |
| `components/*.yaml` | same 9 components (vLLM, container, cluster, gpu0-3, container_loadtest, cluster_loadtest), `system:` swapped |
| `telemetry/prometheus.yaml` | identical 108 metrics, `system:`/`name:` swapped (`Prometheus_11_GPT_OSS_20B_TPS_Peak`) — every metric the goal/constraints reference (`prefill_token_throughput`, `decode_token_throughput`, `active_gpus`, `time_to_first_token_p95`, `inter_token_latency_p95`) is produced here |
| `11-GPT-OSS-20B-TPS-Peak-Workflow.yaml` | paths under `studies/11-gpt-oss-20b-tps-peak/`, same operators/timeouts |
| `11-GPT-OSS-20B-TPS-Peak.yaml` | **two edits only**: `windowing.stability.resolution: 15s` (window = 90s = exactly one concurrency level, since each level now lasts 90s instead of 300s) and optimize **60/12** instead of 200/40 (~11 h instead of ~10 days). Goal, constraints, 14 parameters, domains, 5 `parameterConstraints` and the baseline are byte-identical to study 10 |
...
`/work/vllm-benchmark/studies/11-gpt-oss-20b-tps-peak/akamas/id_rsa` on the toolbox
