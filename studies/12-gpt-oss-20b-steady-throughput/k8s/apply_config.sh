#!/bin/bash
# Apply-config step for 12-gpt-oss-20b-steady-throughput (runs on the toolbox host via the
# Akamas Executor, after FileConfigurator rendered 01-deployment_template.yaml).
set -e
DEPLOY_FILE=/work/vllm-benchmark/studies/12-gpt-oss-20b-steady-throughput/k8s/01-deployment.yaml
