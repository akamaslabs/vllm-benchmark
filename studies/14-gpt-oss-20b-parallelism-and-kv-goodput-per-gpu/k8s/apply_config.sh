#!/bin/bash
# Apply-config step for 13-gpt-oss-20b-tp-goodput-per-gpu (runs on the toolbox host via the
# Akamas Executor, after FileConfigurator rendered 01-deployment_template.yaml).
set -e
DEPLOY_FILE=/work/vllm-benchmark/studies/13-gpt-oss-20b-tp-goodput-per-gpu/k8s/01-deployment.yaml
...
# manifest's last parameterConstraint) yet the trial burned the whole 20-minute progress
# deadline in CrashLoopBackOff before failing. A healthy vLLM start never restarts, so once
