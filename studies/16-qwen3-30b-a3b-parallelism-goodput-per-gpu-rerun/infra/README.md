...
> Copied from `14-gpt-oss-20b-parallelism-and-kv-goodput-per-gpu/infra/` on 2026-09-17 for
> `15-qwen3-30b-a3b-parallelism-goodput-per-gpu` (same cluster, same `llm-serving-l4`
> node group, no new node group needed — only the model changes). **The node is shared
> with study 14, which was still RUNNING on it when this study was scaffolded: study 15
> cannot start, and its smoke test cannot run, until that study is finished or stopped.** The
...
1. **The model-cache EBS volume is pinned to one Availability Zone.** Study 9's
   ...
   **us-east-2c**; the `llm-serving-l4` node group spans us-east-2a/b/c. A node that comes
   ...
   this study therefore uses its **own claim, `vllm-model-cache-qwen3moe`** (80Gi — the
   Qwen3-30B-A3B-Instruct-2507-FP8 checkpoint is 31.18 GB, against gpt-oss-20b's 12.8 GiB)
...
```bash
cd studies/15-qwen3-30b-a3b-parallelism-goodput-per-gpu/infra/eks
