...
`model` property selects this label. Token counters come from the `usage` block vLLM
returns, so each prompt and output token is counted once. On the decode instance a
transferred prompt is counted again, but that is not counted here.
...
async def _handle(api, request: Request):
    ...
    # Streaming: always ask the decode instance for the final usage chunk (token counters),
    # and swallow it again if the client did not ask for it.
                        ...
                        if data == b"[DONE]":
                            ...
                            if obj:
                                ...
                                if obj.get("usage") and not obj.get("choices"):
                                    _record_usage(obj["usage"])
                                    forward = client_wants_usage
                    if forward:
                        yield event + b"\n\n"
            if buf:
                yield buf
