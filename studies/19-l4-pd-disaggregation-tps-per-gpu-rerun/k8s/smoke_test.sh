...
case "${1:-}" in
  up)
    ...
    sed -e "s/\${pd_topology.pd_prefill_instances}/$P/" -e "s/\${pd_topology.pd_decode_instances}/$D/" \
        -e "s/\${pd_topology.pd_kv_connector}/NixlConnector/" -e "s/\${pd_topology.pd_kv_buffer_device}/$BUF/" \
        -e "s/\${vllm_prefill.gpu_memory_utilization}/0.9/" -e "s/\${vllm_prefill.max_num_seqs}/$PS/" \
        -e "s/\${vllm_prefill.max_num_batched_tokens}/$PM/" -e "s/\${vllm_prefill.kv_cache_dtype}/$PK/" \
        -e "s/\${vllm_decode.gpu_memory_utilization}/0.9/" -e "s/\${vllm_decode.max_num_seqs}/$DS/" \
        -e "s/\${vllm_decode.max_num_batched_tokens}/$DM/" -e "s/\${vllm_decode.kv_cache_dtype}/$DK/" \
