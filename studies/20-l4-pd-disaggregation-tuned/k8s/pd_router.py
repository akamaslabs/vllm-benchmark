...
Study 20 addition: per-instance KV cache capacity/usage as NUMBERS. vLLM publishes the
capacity only as a string label of vllm:cache_config_info (kv_cache_size_tokens), and
PromQL cannot turn a label into a value. Every 15 s the router reads each backend's
/metrics and republishes vllm_kv_cache_capacity_tokens / _used_tokens / _capacity_bytes /
_used_bytes with model_name=<role served name> and vllm_instance=<prefill-i|decode-j>. These are
the series the vLLM pack's kv_cache_* metrics read; studies 9-16 had a sidecar that did
the same for a single vLLM.
...
SERVED_BASE = os.environ.get("PD_SERVED_BASE", "qwen3-8b")
# KV bytes per token in bf16 (Qwen3-8B: 36 layers x 8 KV heads x 128 x 2 (K,V) x 2 B); fp8 halves it.
KV_BYTES_PER_TOKEN = int(os.environ.get("PD_KV_BYTES_PER_TOKEN_BF16", "147456"))
...
# Not "endpoint": Prometheus sets its own `endpoint` target label on the router scrape
# ("http") and would rename ours to exported_endpoint.
KV_LABELS = ["model_name", "vllm_instance"]
KV_CAP = Gauge("vllm_kv_cache_capacity_tokens", "KV cache capacity in tokens, per vLLM instance.", KV_LABELS)
KV_USED = Gauge("vllm_kv_cache_used_tokens", "KV cache tokens in use (capacity x vllm:kv_cache_usage_perc).", KV_LABELS)
KV_CAP_B = Gauge("vllm_kv_cache_capacity_bytes", "KV cache capacity in bytes, per vLLM instance.", KV_LABELS)
...
async def _kv_poll():
    while True:
        for inst, url, role in BACKENDS:
            try:
                text = (await client.get(url + "/metrics", timeout=5)).text
            except httpx.HTTPError:
                continue
            cap = dtype = usage = None
            for line in text.splitlines():
                if line.startswith("vllm:cache_config_info{"):
                    m = _SIZE_RE.search(line)
                    cap = int(m.group(1)) if m else cap
                    d = _DTYPE_RE.search(line)
                    dtype = d.group(1) if d else dtype
                elif line.startswith("vllm:kv_cache_usage_perc{"):
                    try:
                        usage = float(line.rsplit(" ", 1)[1])
                    except ValueError:
                        pass
            if cap is None:
                continue
            bpt = KV_BYTES_PER_TOKEN / 2 if (dtype or "").startswith("fp8") else KV_BYTES_PER_TOKEN
            KV_CAP.labels(role, inst).set(cap)
            KV_CAP_B.labels(role, inst).set(cap * bpt)
            if usage is not None:
                KV_USED.labels(role, inst).set(cap * usage)
                KV_USED_B.labels(role, inst).set(cap * usage * bpt)
...
async def _start_kv_poll():
    asyncio.create_task(_kv_poll())
